...
<?php
	...
	?>
		<tr>
			...
			<?php
				$checkEnrolM = dbquery("SELECT * FROM studenroll INNER JOIN student on enrol_stud_no=stud_no WHERE (enrol_sy='".$_GET['enrol_sy']."' AND enrol_level='".$dataSections['section_level']."' and (enrol_graddate  > '".$dateLimit."' and enrol_graddate <= '".$dateLimit."') AND stud_gender='MALE' AND enrol_status2='DROPPED OUT')");
				$countEnrolM = dbrows($checkEnrolM);
				$checkEnrolF = dbquery("SELECT * FROM studenroll INNER JOIN student on enrol_stud_no=stud_no WHERE (enrol_sy='".$_GET['enrol_sy']."' AND enrol_level='".$dataSections['section_level']."' and (enrol_graddate  > '".$dateLimit."' and enrol_graddate <= '".$dateLimit."') AND stud_gender='FEMALE' AND enrol_status2='DROPPED OUT')");
				$countEnrolF = dbrows($checkEnrolF);
				$checkEnrolT = dbquery("SELECT * FROM studenroll INNER JOIN student on enrol_stud_no=stud_no WHERE (enrol_sy='".$_GET['enrol_sy']."' AND enrol_level='".$dataSections['section_level']."' and (enrol_graddate  > '".$dateLimit."' and enrol_graddate <= '".$dateLimit."') AND enrol_status2='DROPPED OUT')");
				$countEnrolT = dbrows($checkEnrolT);
			?>
			<td align="right" width="2%" bgcolor="lightgray"><?php echo number_format($countEnrolM,0);?></td>
			<td align="right" width="2%" bgcolor="lightgray"><?php echo number_format($countEnrolF,0);?></td>
